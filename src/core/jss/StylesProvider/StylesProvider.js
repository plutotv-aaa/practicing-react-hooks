...
export default function StylesProvider(props) {
  ...
  if (process.env.NODE_ENV !== "production") {
    if (typeof window === "undefined" && !context.sheetsManager) {
      console.error(
        "UI: you need to use the ServerStyleSheets API when rendering on the server."
      );
    }
  }
  ...
  if (process.env.NODE_ENV !== "production") {
    if (context.jss.options.insertionPoint && injectFirst) {
      console.error(
        "UI: you cannot use a custom insertionPoint and <StylesContext injectFirst> at the same time."
      );
    }
  }
  ...
  if (process.env.NODE_ENV !== "production") {
    if (injectFirst && localOptions.jss) {
      console.error(
        "UI: you cannot use the jss and injectFirst props at the same time."
      );
    }
  }
...
StylesProvider.propTypes = {
   ...
   * If you want to override UI's styles, set this prop.
   */
